...
import MainTopNavbar from '../../components/layout/AegisNavbar';
import DriverLogoutMenu from '../../components/driver/DriverLogoutMenu';
import { useAuth } from '../../context/AuthContext';
import { useLocation } from '../../context/LocationContext';
import { Theme } from '../../theme';
import { fraudApi } from '../../services/api';
import LocationErrorCard from '../../components/driver/LocationErrorCard';
...
export default function DriverLiveRiskMapboxScreen({ navigation }: any) {
  ...
  useEffect(() => {
    if (!hasValidLocation) {
      ...
      setRiskError(location.error ?? null);
      return;
    }
    ...
    const fetchRiskData = async () => {
      ...
      try {
        ...
        const rawErrorMessage = error?.message ?? 'Failed to load risk data';
        const errorMessage = isUnauthorizedError(rawErrorMessage)
    ...
    fetchRiskData();
  }, [driverLat, driverLon, hasValidLocation, location.error]);
  ...
  useEffect(() => {
    ...
    const fetchInterval = setInterval(async () => {
      try {
        ...
        const rawErrorMessage = error?.message ?? 'Failed to refresh risk data';
        const errorMessage = isUnauthorizedError(rawErrorMessage)
          ? 'Session expired or unauthorized. Please log in again.'
          : rawErrorMessage;
        setRiskError(errorMessage);
  ...
  }, [driverLat, driverLon, hasValidLocation]);
  ...
  const sendToWebView = useCallback((payload: Record<string, unknown>) => {
...
<html>
  ...
  }, [driverLat, driverLon, hasValidLocation, sendToWebView]);
  ...
  if (isLoadingRisk) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <MainTopNavbar onProfile={() => setProfileMenuVisible(true)} light />
  ...
  if (!riskMap || Object.keys(riskMap).length === 0) {
    const isAuthError = isUnauthorizedError(riskError);
    const displayError = riskError ?? location.error;
    const errorType = getLocationErrorType(displayError);
      ...
      return (
        <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.centeredContainer}>
            ...
            <Text style={styles.emptyTitle}>Session Unauthorized</Text>
            <Text style={styles.emptySubtitle}>
              Your session appears expired. Please sign in again to load live risk data.
            </Text>
            <TouchableOpacity
      ...
      return (
        <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.centeredContainer}>
            <LocationErrorCard
              errorType={errorType}
              message={displayError || 'Unknown error'}
