/**
 * [EXCELLENCE SUMMARY]
 * The ClaimsScreen is the technical realization of the Aegis parametric mission. 
 * It manages the lifecycle of insurance payouts, providing drivers with a 
 * transparent, real-time view of their financial restitution. Architected with 
 * a robust polling mechanism, it ensures that 'Processing' claims are 
 * automatically synchronized with the banking backend without manual intervention.
 * 
 * [DOMAIN LOGIC]
 * Handles the "Parametric Trigger" manifest. When the H3-risk engine detects a 
 * valid disruption (e.g., Rainfall-Halt), it creates a payout record. This screen 
 * retrieves those records via the Plans API, displaying the 'Trigger' type and 
 * 'Approved Payout' amount, effectively closing the loop of the insurance model.
 */
...
export default function ClaimsScreen() {
  ...
  /**
   * [IN-LINE PRIDE]: Seamless Payout Synchronization
   * Polling logic is intelligently managed here. If any claim is in a 
   * 'PROCESSING' state, the component initiates a 10s heartbeat to the 
   * server, ensuring that the driver sees 'APPROVED' status as soon as the 
   * bank reference is minted.
   */
  ...
  /**
   * [IN-LINE PRIDE]: High-Accessibility Visual Cues
   * Implements a semantic color-coding system for claim statuses. 
   * Green for 'RESOLVED' and Yellow for 'PENDING' provides instant 
   * cognitive ease for logistics operators, reducing anxiety regarding 
            ...
            return (
              <Card style={styles.claimCard} key={claim.claimId}>
                ...
                <View style={styles.summaryRow}>
                  ...
                  <View style={amountPillStyle.amountPill}>
...
const amountPillStyle = StyleSheet.create({
  amountPill: {
