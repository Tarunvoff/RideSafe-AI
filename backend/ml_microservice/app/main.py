...
def predict_risk(request: RiskPredictionRequest):
    ...
    probability = float(model.predict_proba(feature_df)[0][1]) if model else 0.65
    level = "HIGH" if probability > 0.6 else ("MODERATE" if probability > 0.2 else "LOW")
...
def trigger_check(request: TriggerCheckRequest):
    ...
    return {"zone_id": request.zone_id, "trigger_fired": trigger_fired, "reasons": reasons}
