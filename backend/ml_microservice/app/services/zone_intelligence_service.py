...
def calculate_zone_intelligence(zone_id: int, db: Session):
    ...
    try:
        ...
        latest_env = db.query(EnvironmentData).filter(EnvironmentData.zone_id == zone_id)\
    ...
    if latest_env:
        ...
        active_riders_count = latest_env.active_riders_count if latest_env.active_riders_count is not None else active_riders_count
    else:
    ...
    if model:
        # Assuming the model returns probability for class '1' at index 1
