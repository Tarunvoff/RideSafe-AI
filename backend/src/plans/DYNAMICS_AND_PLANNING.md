...
- **Seeded Consistency**: Earnings are generated using a `SeededRandom` algorithm tied to the `internalDriverId` and `weekKey`. This ensures that a worker sees the same, stable income throughout the week—mirrored across both the backend and mobile dashboard.
