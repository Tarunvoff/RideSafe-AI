...
class ZoneAggregator:
    def __init__(self):
        ...
        self.zone_last_coords: Dict[str, tuple[float, float]] = {}
    ...
    async def process_ping(self, h3_cell: str, driver_id: str, ts: float, lat: float | None = None, lng: float | None = None):
        ...
        if lat is not None and lng is not None:
            self.zone_last_coords[h3_cell] = (lat, lng)
    ...
    async def _get_ml_zone_data(self, client: httpx.AsyncClient, h3_cell: str, rider_count: int) -> dict | None:
        ...
        """
        latlng = self.zone_last_coords.get(h3_cell)
        ...
        payload = {
            ...
            "Ct": 0.6,
            "M": 0.1,
            "active_orders": rider_count,
            "active_riders": rider_count,
            "demand_ratio": 1.0,
        }
        try:
